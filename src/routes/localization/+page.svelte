<script lang="ts">
  ...
  let selectedGameLang: string = (typeof localStorage !== 'undefined'
    ? localStorage.getItem('thunderstrike_game_lang') ?? 'English'
    : 'English');
  ...
  async function handleGameLangChange(newLang: string) {
    ...
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('thunderstrike_game_lang', newLang);
    }
    currentPage = 1;
  ...
  function handleEdit(key: string, val: string, original: string) {
    if (val === original) {
      // Text matches original – no diff needed
      delete pendingDiffs[key];
    } else {
      // Empty string is intentional (user cleared the field) – keep as diff
